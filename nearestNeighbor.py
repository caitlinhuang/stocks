...
def antiError():
    ...
    slopeSent = numpy.polyfit(numpy.array(sent).astype(float),
    numpy.array(returns).astype(float), 1)
    slopeRel = numpy.polyfit(numpy.array(rel).astype(float),
    numpy.array(returns).astype(float), 1)
...
def nearestNeighbor(dataIn, steps, randomness, weights = [1]):
    ...
    difference = train.iloc[minIndex].get("returnsClosePrevRaw10")/10 * \
